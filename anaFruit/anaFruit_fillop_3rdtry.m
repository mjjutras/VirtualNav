...
%     figure;plot(1:size(data.sampleinfo,1),data.sampleinfo);title(sesttl)
    ...
    for invlop = 1:length(prvbanalp)
        % choose only "invisible banana trials" with at least 3 preceding visible bananas 
...
bantim_himem = bantimindall(invprmnrlbadsel(memindhi));
bantim_lomem = bantimindall(invprmnrlbadsel(memindlo));

% figure;hold on
% scatter3([bantimindall(invprmnrlbadsel(memindhi)); bantimindall(invprmnrlbadsel(memindlo))], ...
%     [avgbandispertrl(invprmnrlbadsel(memindhi)); avgbandispertrl(invprmnrlbadsel(memindlo))], ...
%     [avgwaldispertrl(invprmnrlbadsel(memindhi)); avgwaldispertrl(invprmnrlbadsel(memindlo))], ...
