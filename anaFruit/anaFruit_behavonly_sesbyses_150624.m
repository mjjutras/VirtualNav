...
% save(fullfile(WSdir,'anaFruit_behav_150624.mat'))

% load variables
load(fullfile(WSdir,'anaFruit_behav_150624.mat'))
...
% this method uses gaussian smoothing, which may not be the correct way to
% visualize, since the values shouldn't drop to zero off of the peak
...
mem3d = []; % memory performance
beg3d = []; % starting distance
fnd3d = []; % index of found bananas
epldis3d = []; % excess path length, difference
eplrat3d = []; % excess path length, ratio
memnrm3d = []; % memory normalized by starting distance
for k=1:length(memdst)
    
    posn = imfilter(hist3(banposall(invprmnrlsel(k),:),'Edges',{x,x}),fspecial('disk',10));
    posn(posn~=max(max(posn))) = nan; posn(~isnan(posn))=1;
    
    % multiply by scaling factor to correct for smoothing (to plot correct amplitudes)
    lat3d(k,:,:) = posn(1:end-1,1:end-1)*bantimindall(invprmnrlsel(k));
    dis3d(k,:,:) = posn(1:end-1,1:end-1)*avgbandispertrl(invprmnrlsel(k));
    wal3d(k,:,:) = posn(1:end-1,1:end-1)*avgwaldispertrl(invprmnrlsel(k));
%     mem3d(k,:,:) = posn(1:end-1,1:end-1)*memdst(k);
%     beg3d(k,:,:) = posn(1:end-1,1:end-1)*begdis(k);
    fnd3d(k,:,:) = posn(1:end-1,1:end-1)*banfndall(invprmnrlsel(k));
    epldis3d(k,:,:) = posn(1:end-1,1:end-1)*excpthdif(invprmnrlsel(k));
    eplrat3d(k,:,:) = posn(1:end-1,1:end-1)*excpthnrm(invprmnrlsel(k));
    eplind3d(k,:,:) = posn(1:end-1,1:end-1)*excpthind(invprmnrlsel(k));
    memnrm3d(k,:,:) = posn(1:end-1,1:end-1)*(memdst(k)/begdis(k));
...
lat2d = squeeze(nanmedian(lat3d,1));
dis2d = squeeze(nanmedian(dis3d,1));
wal2d = squeeze(nanmedian(wal3d,1));
mem2d = squeeze(nanmedian(mem3d,1));
beg2d = squeeze(nanmedian(beg3d,1));
fnd2d = squeeze(nanmean(fnd3d,1));
epldis2d = squeeze(nanmedian(epldis3d,1));
eplrat2d = squeeze(nanmedian(eplrat3d,1));
eplind2d = squeeze(nanmedian(eplind3d,1));
memnrm2d = squeeze(nanmedian(memnrm3d,1));
...
figure;imagesc(ctrs,ctrs,mem2d);xlim([-11.2 11.2]);ylim([-11.2 11.2]);colormap jet;title('Memory performance by banana location');colorbar
figure;imagesc(ctrs,ctrs,beg2d);xlim([-11.2 11.2]);ylim([-11.2 11.2]);colormap jet;title('Starting distance by banana location');colorbar
figure;imagesc(ctrs,ctrs,fnd2d);xlim([-11.2 11.2]);ylim([-11.2 11.2]);colormap jet;title('Probability of finding banana');colorbar
figure;imagesc(ctrs,ctrs,epldis2d);xlim([-11.2 11.2]);ylim([-11.2 11.2]);colormap jet;title('Excess path length (difference) by banana location');colorbar
figure;imagesc(ctrs,ctrs,eplrat2d);xlim([-11.2 11.2]);ylim([-11.2 11.2]);colormap jet;title('Excess path length (ratio) by banana location');colorbar
figure;imagesc(ctrs,ctrs,eplind2d);xlim([-11.2 11.2]);ylim([-11.2 11.2]);colormap jet;title('Excess path length (index) by banana location');colorbar
figure;imagesc(ctrs,ctrs,memnrm2d);xlim([-11.2 11.2]);ylim([-11.2 11.2]);colormap jet;title('Normalized memory by banana location');colorbar
