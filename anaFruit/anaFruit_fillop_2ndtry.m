...
prvbanalpall = cell(1); % alpha of bananas preceding each prime inv. banana
...
%     figure;plot(1:size(data.sampleinfo,1),data.sampleinfo);title(sesttl)
    ...
    prvbanalpall(c:length(invprmind)+c-1,1) = prvbanalp;
...
badtrl = [9 15 17 19 27 29 56 65 66 105 109];
...
trlind = (1:length(timsel))'; % same size as memdst right now
