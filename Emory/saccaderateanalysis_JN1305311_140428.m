...
data_eyeset=strcat('R:\Buffalo lab\Virtual Navigation\Free Foraging\NEX Files\',fid,'.nex');
...
save('C:\Users\michael.jutras\Documents\MATLAB\Virtual\FT data\JN1305311_sacdat140428.mat','data_eye')
...
for frqlop = 1:length(freqs)
    ...
    sacratperfrq(frqlop) = nanmedian(sacpersec_smooth(logical(unionvector(frqlop,:))));

    n = histc(sacpersec_smooth(logical(unionvector(frqlop,:))),edges);
    sac_hist(frqlop,:) = n;
...
r = length(sacpersec_smooth);edges = 0:0.1:8; [n, bin] = histc(sacpersec_smooth,edges);figure;bar(edges,n/r,'histc')
...
for binlop = 1:length(edges)-1
    pepbin(binlop,:)=mean(unionvector(:,bin==binlop),2);
end
