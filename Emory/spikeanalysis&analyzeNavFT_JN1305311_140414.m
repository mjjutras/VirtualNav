...
[times_eye, id_eye, x_eye, y_eye] = textread('R:\Buffalo Lab\Virtual Navigation\Free Foraging Data - Emory\parsed log files\session_1191\bzflag2.par', '%f%s%f%f');
[times_ava, id_ava, x_ava, y_ava] = textread('R:\Buffalo Lab\Virtual Navigation\Free Foraging Data - Emory\parsed log files\session_1191\avatar.par', '%f%s%f%f');
...
delay_var = 8; % ascertained this from visual inspection of eye data
...
for trllop=[3 10 30 50]
    if ~isempty(trlinf{trllop})
        plex_trl = data.cfg.previous{1}.trl(trllop,1:2);

        figure;hold on
        plot(plex_trl(1):plex_trl(2),data.trial{trllop}(10,:))
...
save('C:\Users\michael.jutras\Documents\MATLAB\Virtual\FT data\JN1305311_navdat140425.mat','data')

%% look at cell firing by location in environment
...
%% look at cell firing by orientation direction (heading)

% % convert direction data to radians, and point in the right direction
% dirdatrad = mod(dirdat + 90, 360) * pi/180;

spkind = find(strncmp('sig',data.label,3));
spkdir = cell(1,length(spkind));
spkvel = cell(1,length(spkind));
trltim = cell(1,length(spkind));
lastgoodtrial = [18 10 14 11 18];
for spklop = 1:length(spkind)
    for trllop = 1:lastgoodtrial(spklop)
        spkdir{spklop} = [spkdir{spklop} [data.trial{trllop}(spkind(spklop),:); mod(data.dirdat{trllop} + 90, 360) * pi/180; data.dirdat{trllop}]];
        spkvel{spklop} = [spkvel{spklop} data.veldat{trllop}];
        trltim{spklop} = [trltim{spklop} data.sampleinfo(trllop,1):data.sampleinfo(trllop,2)];
    end
end

edges = 0:pi/12:2*pi;
avgfrate_binned = cell(1,length(spkind));
for spklop = 1:length(spkdir)
    for binlop = 1:length(edges)-1
        
        avgfrate_binned{spklop}(binlop) = mean(spkdir{spklop}(1,spkdir{spklop}(2,:)>=edges(binlop) & spkdir{spklop}(2,:)<edges(binlop+1)));
        
    end
end

% make the polar plot
for spklop = 1:length(avgfrate_binned)
    c=1;
    clear t r
    for k=1:length(edges)-1
        ...
        t(c+2:c+3) = edges(k+1);
        r(c) = 0;
        r(c+1:c+2) = avgfrate_binned{spklop}(k)*1000;
        r(c+3) = 0;
        c=c+4;
    end
    figure;polar(t,r)
...
%% use spkvel and spkdir to divide behavior into periods of turning and periods of traversing
% then look at pepisode between the two conditions

load('C:\Users\michael.jutras\Documents\MATLAB\Virtual\FT data\JN1305311_navdat140425.mat')

freqs = (2^(1/8)).^(8:42);

pep=load('C:\Users\michael.jutras\Documents\MATLAB\Virtual\Pepisode & CFC\JN130531.1-sorted_pEpisode140428.mat');

pepisode1 = cell(1,length(pep.unionvector_all));
pepisode2 = cell(1,length(pep.unionvector_all));
lfp1 = cell(1,length(pep.unionvector_all));
lfp2 = cell(1,length(pep.unionvector_all));
for trllop = 1:length(data.trial)
    
    timarr = data.sampleinfo(trllop,1):data.sampleinfo(trllop,2);
    
    dirdif = diff(data.dirdat{trllop});
    
    difind1 = dirdif>0.002 | dirdif<-0.002; % monkey is turning
    difind2 = dirdif<0.002 & dirdif>-0.002; % monkey is moving forward
    
    difind1(data.veldat{trllop}>0.15)=0;
    difind2(data.veldat{trllop}>0.15)=1;

%     figure;hold on
%     plot(timarr,data.veldat{trllop},'b')
%     plot(timarr(1:end-1),data.veldat{trllop}(1:end-1).*difind1,'r')

    
%     [u,v] = pol2cart(mod(data.dirdat{trllop} + 90, 360) * pi/180,data.veldat{trllop});
%     figure
%     quiver(data.posdat{trllop}(1,1:100:end),data.posdat{trllop}(2,1:100:end),u(1:100:end),v(1:100:end))
%     xlim([-5 5])
%     ylim([-5 5])
%     title(['Trial ' num2str(trllop)])

    
    [~,ia]=intersect(pep.trlind,timarr/1000);
    
    lfpind = strmatch('AD',data.label);
    
    for lfplop = 1:length(pep.unionvector_all)
    
        pepisode1{lfplop} = cat(2, pepisode1{lfplop}, pep.unionvector_all{lfplop}(:,ia(difind1)));
        pepisode2{lfplop} = cat(2, pepisode2{lfplop}, pep.unionvector_all{lfplop}(:,ia(difind2)));
        
        lfp1{lfplop} = cat(2, lfp1{lfplop}, data.trial{trllop}(lfpind(lfplop),difind1));
        lfp2{lfplop} = cat(2, lfp2{lfplop}, data.trial{trllop}(lfpind(lfplop),difind2));
...
for lfplop = 1:length(pepisode1)
    figure; hold on
    plot(freqs,mean(pepisode1{lfplop},2),'r')
    plot(freqs,mean(pepisode2{lfplop},2),'b')
end
